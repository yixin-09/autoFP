...
let rec move_out_calltable (fid:int)(s:Cil.stmt) =
	(match s.skind with 
		  ...
		  | Loop (b,l,s1,s2) -> (if l.line < fid then begin List.iter (move_out_calltable fid) b.bstmts;
	          ...
	          | Instr il-> if (!stmt_id = s.sid) then begin List.iter (save_to_calltable fid) il; end else List.iter (move_out_calltable_i fid) il;
	          | _ -> ();
	)
...
let print_message (fn:string)  =
	let fout = open_out_gen [Open_append;Open_text;Open_creat] 0b111100100 fn in 
	Printf.fprintf fout "variable ";
	close_out fout;;
...
let rec print_call (e:Cil.exp) = 
	...
	| Lval (vr) -> (if Hashtbl.mem calltable vr then begin 
				print_message filename ;print_exp filename exPrinter e; 
...
let rec print_val (e:Cil.exp) = 
	...
	let filename = "val"^".txt" in
	(match e with
	| BinOp (bp,e1,e2,ty) -> print_val e1;print_val e2;
	| Lval (vr) -> print_exp filename valPrinter e;
	| UnOp (unop,exp,ty) -> print_val exp;
	| CastE(ty,e) -> print_val e;
	| _ ->();)
...
(*go though the program, get the sid of the fid*)
class sidVisitor (file : Cil.file) 
                  (fid : int) 
                  = object
  inherit nopCilVisitor
  method vstmt s = ChangeDoChildrenPost(s, fun s -> (let cur_l = Cil.get_stmtLoc s.skind in 
	          (if (cur_l.line<=fid) then begin (if(!stmt_id < s.sid) then begin stmt_id := s.sid; s; end else s;)
		end else  s;)) 
    ) 
end 
