...
static Bool 		clo_ignoreLibraries 	= True;
...
static Char* mpfrToString2(Char* str, mpfr_t* fp) {
	Int sgn = mpfr_sgn(*fp);
	if (sgn >= 0) {
		str[0] = ' '; str[1] = '0'; str[2] = '\0';
	} else {
...
static void printError(Char* varName, ULong addr, Bool conditional) {
	...
	if (svalue) {
		...
		mpfrToString2(mpfrBuf, &rel);
		VG_(umsg)("(%s) %s RELATIVE ERROR:   %s\n", typeName, varName, mpfrBuf);		
   		{Char fname[256];
		VG_(sprintf)(fname, "%s_relative_error", "statistic");
		getFileName(fname);
		SysRes fileRes = VG_(open)(fname, VKI_O_CREAT|VKI_O_TRUNC|VKI_O_RDWR, VKI_S_IRUSR|VKI_S_IWUSR);
		Int file = sr_Res(fileRes);
		VG_(sprintf)(formatBuf, "%s\n",mpfrBuf);
		my_fwrite(file, (void*)formatBuf, VG_(strlen)(formatBuf));
		}
		UInt n_values = 0;
		MeanValue** values = VG_(HT_to_array)(meanValues, &n_values);
		VG_(ssort)(values, n_values, sizeof(VgHashNode*), compareMVIntroError);
		VG_(describe_IP)(values[0]->key, description, DESCRIPTION_SIZE);
		Char fname[256];
   		VG_(sprintf)(fname, "%s_location","max");
   		getFileName(fname);
		SysRes fileRes = VG_(open)(fname, VKI_O_CREAT|VKI_O_TRUNC|VKI_O_RDWR, VKI_S_IRUSR|VKI_S_IWUSR);
		Int file = sr_Res(fileRes);
   		VG_(sprintf)(formatBuf, "%s\n",description);
